...
<!-- <div class="input-group">
 ...
 <div class="dropdown show" id="menu">
  ...
  <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
   <button onclick="select()" class="dropdown-item">SELECT MULTIPLE</button>
    <a class="dropdown-item" href="./ContactServlet?action=sorty&id=1">Sort A-Z</a>
    <a class="dropdown-item" href="./ContactServlet?action=sorty&id=2">Sort Z-A</a>
  </div>
</div>
...
<table class="table table-hover table-dark">
  ...
  try{
    ...
    <tr>
      ...
      <form action="./ContactServlet" id="f">
      <input type="hidden" class="select" name="check" value=<%=item.getId()%>>  
    </tr>
    <%}}catch(Exception e){}%>
    <input type="hidden" name="action" value="multi">
    <button onclick="find()" class="delt">DELETE</button>
 </form>
  </tbody>
</table>
...
function select(){
	...
	console.warn(t[0].innerHTML)
	if(t[0].type=="hidden"){
	for(let i=0;i<t.length;i++){
		t[i].type="checkbox";
	}
	document.querySelector('.delt').classList.add('showbutton');
		...
		document.querySelector('.delt').classList.remove('showbutton');
		}
