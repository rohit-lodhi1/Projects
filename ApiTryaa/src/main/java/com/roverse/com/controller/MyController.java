...
public class MyController {
	...
	@GetMapping("/employee/page/{page}/{size}")
	public Page<Employee> getEmployee(@PathVariable("page") String page,@PathVariable("size") String size){
		System.out.println(page+size);
		return emp.getEmployeeByPage(Integer.parseInt(page),Integer.parseInt(size));
	}
	...
	public ResponseEntity<HttpStatus> deleteEmployee(@PathVariable String employeeId) {
		try {
		     if(emp.deleteEmployeeByID(Integer.parseInt(employeeId))>0)
